...
- ISO monetary validation first preserves and checks the supplied `BigDecimal` scale. Validate a non-negative amount jointly with a JDK-supported, fixed-fraction currency and require the supplied scale not to exceed the currency fraction digits. Then normalize upward to the currency's exact scale by appending zeros. Zero and negative scales are valid; rounding, downscaling, and removal of supplied precision are forbidden.

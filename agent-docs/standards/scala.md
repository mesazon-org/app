...
- Name a binding that directly holds a domain-specific model from that model's complete name in lower camel case, then append qualifiers such as `Expected`. Utility and infrastructure outputs are the exception: their types are intentionally generic, so name each binding for the concrete domain input or operation it represents, such as `customerBookPhotoScanOutput` for `FileScannerScanOutput`. Do not apply exact-type naming mechanically when it would discard the context that distinguishes one generic output from another.

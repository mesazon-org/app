...
- Functional: `FileServiceSpec` has one `extractCustomers` block proving image and spreadsheet dispatch from `FileScannedPath`, converted `CsvValidatedPath` forwarding, scanner/spreadsheet/AI failure propagation, and no downstream call after earlier failure. `Mocks.AIClientMock` records path-native generic calls because ScalaMock cannot mock those method shapes directly.
...
- Unit: `FileScannerSpec` has one successful table covering every supported media type and focused failures for extension, detected-content mismatch, read failure, and actual byte-cap enforcement. `SpreadsheetToolSpec` covers first-sheet-only `.xlsx`, legacy `.xls`, valid CSV reuse, malformed CSV rejection, and unexpected media types.
...
**Status (photo, CSV, Excel extraction):** the unified path-based implementation is complete. `POST /extract/customer-book` accepts all supported sources with required `X-File-Name`; the two old routes and stream-based service/client methods are removed. The live ten-photo golden suite remains manual and its historical exact baselines are not treated as automated release proof.

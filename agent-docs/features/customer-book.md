...
**Status (photo, CSV, Excel extraction):** photo extraction is implemented. Verification on 2026-09-13: both gateway test modules compile, 28 client/FileService tests pass, all 30 `FileApiSpec` acceptance cases pass through the shared parent harness (including all eight photo-extraction cases), and lint passes. The live ten-photo suite generated and decoded every response but failed all ten exact baselines; the feature is not fully golden-verified. `AIClient` has its own `config/AIClientConfig.scala` and `ai-client` configuration, ordinary and AI JSON concerns live in `json/tapir.scala` and `json/ai.scala`, `Main.scala` provides the client layers, and `POST /extract/customer-book-photo` is routed through `tapir/FileServiceEndpoints.scala`. CSV/Excel extraction: skeleton slice landed on 2026-09-14 — `SupportedMediaType` gained `CSV`/`PLAINTEXT_CSV`/`XLS`/`XLSX`/`spreadsheets`, `AIClient.extract` is renamed and branches on `supportedMediaType` with the text branch stubbed (`ZIO.die`), and a new `AIClientSpec` case pins the desired text-content request shape (red: fails on the intentional stub, not a build or fixture problem; the other 12 `AIClientSpec` cases and all 153 functional tests still pass unchanged). Everything else in this section — the converter, the CSV validity gate, `FileService.extractCustomersFromFile`, the new endpoint, and acceptance coverage — is not started; this section describes the agreed technical design pending implementation.

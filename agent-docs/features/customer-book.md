...
- The raw body is accompanied by three required headers: `X-File-Name-Declared`, `X-Content-Type-Declared`, and `X-File-Size-Declared`. They are validated, compared with the scanned file, and never stored. Missing or malformed headers decode to the existing generic `400 BAD_REQUEST_ERROR`; mismatch details use the same public error and remain visible only in server logs.
- New domain newtypes in `Newtypes.scala` are `FileNameDeclared` (`String` with `NonEmptyTrimmed`), `ContentTypeDeclared` (`String` with `NonEmptyTrimmed`; media-type syntax is validated by `FileScanner.scanV1`), and `FileSizeDeclared` (non-negative `Long`). The endpoint requires all three; none is an `Option`.
- `FileScanner.scan` and all existing logo/catalogue callers remain unchanged. A new `scanV1(fileByteStream, fileNameDeclared, contentTypeDeclared, fileSizeDeclared, supportedMediaTypes, maxFileBytes)` performs the same bounded disk write and content detection independently of the declarations, then validates them against the authoritative result. Supported filename extensions are compared case-insensitively (`jpg` and `jpeg` both map to JPEG); content type ignores case and parameters, and treats `text/csv`/`text/plain` as equivalent CSV declarations; declared size must exactly equal the received byte count. A malformed declaration or mismatch fails with `ServiceError.BadRequestError`; unsupported/unreadable content and actual content over 20 MB keep the existing `ServiceError.InternalServerError` precedence. In particular, actual content over 20 MB stays `500` regardless of the declared size, and the network stream is still fully drained.
- `FileService`'s two extraction methods collapse into one required-metadata method, `extractCustomers(organizationID, fileNameDeclared, contentTypeDeclared, fileSizeDeclared, customerBookByteStream)`. It calls `fileScanner.scanV1` once with `SupportedMediaType.images ++ SupportedMediaType.spreadsheets`, then dispatches on the authoritative detected type: images call `aiClient.extractFromImage`; spreadsheets call `spreadsheetTool.convertValidateCsv` and then `aiClient.extractFromCsv`. Metadata rejection happens before either AI method. `AIClient`, `SpreadsheetTool`, persistence, and retry/timeout policy are unchanged.
- Tests consolidate onto the one endpoint: `GatewayClient` gains one raw-body method whose metadata arguments are optional only so rejection tests can omit each otherwise-required HTTP header; `FileApiSpec` collapses to one `/extract/customer-book` block, proving the middleware matrix once, image/CSV/Excel success, each missing/malformed declaration, each metadata mismatch, and existing unsupported/plain-zip backstops. `FileScannerSpec` owns declaration normalization/matching, mismatch, and actual-oversize precedence. `FileServiceSpec` collapses its two extraction blocks into one and proves image/spreadsheet dispatch plus no AI call after scanner rejection.

#### Planned implementation slices

Every slice is at most three hand-written files and stops for user approval and an EM commit:

1. Skeleton/red for declared metadata scanning: `Newtypes.scala`, `FileScanner.scala`, `FileScannerSpec.scala` — add the three newtypes, the unimplemented `scanV1` signature, and failing behavior tests.
2. Green for declared metadata scanning: `FileScanner.scala`, `FileScannerSpec.scala`, this feature doc — implement and prove matching, normalization, mismatch `400`, unchanged `scan`, and actual-oversize `500` precedence.

...
- Acceptance: not yet added — `it/FileApiSpec.scala`'s `/extract/customer-book-photo` block already exists; the new `/extract/customer-book-file` block, `it/client/GatewayClient.scala`'s new `extractCustomersFromFilePost`, and new text-shaped wiremock mapping(s) are the next slice's work
...
- Unit: `FileScannerSpec` remains unchanged for the photo endpoint because it adds no new size/type-check behavior. `SpreadsheetToolSpec` (`unit/utils/SpreadsheetToolSpec.scala`) merges both former specs' scenarios into one: first-sheet-only on a multi-sheet `.xlsx`, legacy `.xls` reads the same as `.xlsx`, well-formed CSV accepted without conversion, structurally malformed CSV rejected with `ServiceError.InternalServerError.UnexpectedError`. The dedicated schema-only spec was removed by agreement; real OpenAI schema acceptance remains the manual golden suite's responsibility.

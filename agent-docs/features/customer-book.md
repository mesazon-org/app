...
Renamed from `extractFromImage` at the skeleton slice; the trait, its live implementation, and every caller (`FileService`, `Mocks.AIClientMock`, `AIClientSpec`, the manual `ExtractCustomersFromPhotoGoldenSpec`) use `extract` now.
...
- Any other `supportedMediaType` (`CSV`, `PLAINTEXT_CSV`, `XLS`, `XLSX`): new text branch — decode the bytes as UTF-8 and send them as plain message text via `Content.TextContent(text)` (confirmed at the skeleton slice; it encodes as a bare JSON string, distinct from the image branch's `Content.ArrayContent`/`image_url` array shape) instead of an image part. `XLS`/`XLSX` reach this branch already holding POI-converted CSV text, never raw spreadsheet bytes — `AIClient` never touches POI or Commons CSV itself. The text branch is stubbed (`ZIO.die(NotImplementedError)`) as of the skeleton slice; implementation is the next slice.
...
CSV/Excel extraction, as of 2026-09-14: `SupportedMediaType` gained `CSV`/`PLAINTEXT_CSV`/`XLS`/`XLSX`/`spreadsheets`. `AIClient`'s method is renamed `extract` and fully implemented for both branches — the image branch unchanged, and the new text branch (UTF-8 decode, `Content.TextContent`, shared `sendAndDecode` helper) green against a dedicated wiremock mapping; a third `match` arm fails loudly (`ServiceError.InternalServerError.UnexpectedError`) for any media type outside both known groups, rather than silently defaulting to text handling. Apache POI and Apache Commons CSV are added as dependencies, alongside a `log4j-to-slf4j` bridge (pinned to match POI's transitive `log4j-api` version) so POI's internal logging routes through this project's normal Logback pipeline instead of falling back to a console logger. `ExcelToCsvConverter` (`utils/ExcelToCsvConverter.scala`) is fully implemented: `WorkbookFactory.create` auto-detects `.xls`/`.xlsx`, only `getSheetAt(0)` is ever read, rows are written as CSV-shaped text via Commons CSV's `CSVPrinter`, and the result is returned as `FileByteStreamScanned` — the same wrapper a real CSV upload produces. Its unit spec proves both first-sheet-only-on-a-multi-sheet-workbook and legacy-`.xls`-reads-the-same-as-`.xlsx`. `CsvValidator` (`utils/CsvValidator.scala`) is fully implemented: a real Apache Commons CSV structural parse (`CSVParser.parse` plus a forced `getRecords()` read, since the tokenizer is lazy and would otherwise defer a malformed-input error past `validate`'s return), well-formed CSV passes as `Unit`, structurally invalid content fails with `ServiceError.InternalServerError.UnexpectedError`.

`FileService.extractCustomersFromFile` is fully implemented: `fileScanner.scan` against `SupportedMediaType.spreadsheets`, then an exhaustive match on the detected type — `XLS`/`XLSX` through `excelToCsvConverter.convert`, `CSV`/`PLAINTEXT_CSV` through `csvValidator.validate` (pass-through unchanged on success), any other type fails loudly rather than falling through — and either path converges into one `aiClient.extract[ExtractCustomersResponse]` call tagged `SupportedMediaType.CSV`, using the new `extractCustomersFromFileInstructions` prompt. `Main.scala` wires `ExcelToCsvConverter.live`/`CsvValidator.live`. `FileServiceSpec`'s five `extractCustomersFromFile` cases now have real per-scenario mocks: CSV success (asserts the scanned stream and the four required instruction phrases reach `AIClient` unchanged), Excel success (asserts the *converted* stream, not the original XLSX bytes, is what reaches `AIClient`), `FileScanner` failure, `CsvValidator` failure, and `AIClient` failure — each proving `AIClient` is/isn't called as appropriate. The new `POST /extract/customer-book-file` Tapir endpoint/wiring and acceptance coverage (`FileApiSpec`'s new block, `GatewayClient.extractCustomersFromFilePost`, wiremock mappings) are not started; this section describes the agreed technical design pending implementation for those remaining pieces.

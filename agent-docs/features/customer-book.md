...
- `POST /extract/customer-book-file` — binary body is a CSV or Excel file (`SupportedMediaType.spreadsheets`: `CSV`, `XLS`, `XLSX`). New.
...
Everything above is the shipped, two-endpoint design. It is being replaced as described in [epic step 7](../../pages/epics/05-customer-book.md#7-user-extracts-customers-from-a-photo-or-file):
...
- `FileScanner.scan` and all existing logo/catalogue callers remain unchanged. A new `scanV1(fileByteStream, fileNameDeclared, supportedMediaTypes, maxFileBytes)` performs the same bounded disk write, uses the declared filename as a Tika hint, detects the media type, and validates it against the type selected by the filename extension. Supported filename extensions are compared case-insensitively (`jpg` and `jpeg` both map to JPEG). A malformed declaration, unsupported detected content, or mismatch fails with `ServiceError.BadRequestError`; an unreadable stream and actual content over 20 MB keep the existing `ServiceError.InternalServerError`, and the network stream is still fully drained and counted.
- `FileService`'s two extraction methods collapse into one required-metadata method, `extractCustomers(organizationID, fileNameDeclared, customerBookByteStream)`. It calls `fileScanner.scanV1` once with `SupportedMediaType.images ++ SupportedMediaType.spreadsheets`, then dispatches on the detected type: images call `aiClient.extractFromImage`; spreadsheets call `spreadsheetTool.convertValidateCsv` and then `aiClient.extractFromCsv`. Filename rejection happens before either AI method. `AIClient`, `SpreadsheetTool`, persistence, and retry/timeout policy are unchanged.
...
Every slice is at most three hand-written files and stops for user approval and an EM commit:

1. Skeleton/red for declared metadata scanning: `Newtypes.scala`, `FileScanner.scala`, `FileScannerSpec.scala` — add the filename newtype, the unimplemented `scanV1` signature, and failing behavior tests.
2. Green for declared metadata scanning: `FileScanner.scala`, `FileScannerSpec.scala`, this feature doc — implement and prove matching, mismatch `400`, unchanged `scan`, and bounded full-drain handling.
3. Skeleton/red for merged orchestration: `FileService.scala`, `FileServiceSpec.scala` — add the merged method with an unimplemented body and failing dispatch/rejection tests.
4. Green for merged orchestration: `FileService.scala`, `FileServiceSpec.scala`, this feature doc — call `scanV1`, dispatch by detected format, and prove rejected metadata never reaches AI.
5. HTTP red: `GatewayClient.scala`, `FileApiSpec.scala` — add the new required-header client request and failing acceptance cases against the not-yet-routed path.
6. HTTP green: `FileServiceEndpoints.scala`, `FileApiSpec.scala`, this feature doc — replace both old routes with the one raw streaming route, preserve the security gate, and make the consolidated acceptance block pass.
7. Client cleanup: `GatewayClient.scala` — remove the two superseded client methods after no acceptance test uses them.

**Implementation status (2026-09-15):** slices 1–2 are complete. `FileScanner.scanV1` independently performs its bounded temp-file write, full stream drain, actual-content detection, supported-type selection, byte-count construction, and declaration validation through focused private helpers; it does not delegate to the legacy `scan` path. The legacy `scan` implementation and its callers are unchanged. Merged service orchestration and transport remain not implemented.
...
**HIGH risk and rollback:** this is a breaking replacement of two public endpoints at the upload-stream boundary, where the open `EntityLimiter`/Ember oversized-body hang makes an accidental buffering or early-stop change material. The design bounds that risk by leaving `scan` intact, adding `scanV1`, retaining raw Tapir streaming, and excluding the known-hanging HTTP test. There is no schema or stored-data migration; rollback is a code deployment restoring the two old routes and extraction methods. Because there is intentionally no compatibility overlap, clients and gateway must roll forward or back together.

### Media types

`SupportedMediaType` represents every media family with non-empty sets of accepted extensions and MIME types. `CSV` accepts extension `csv` and both `text/csv` and `text/plain` (Tika's common fallback detection for a real CSV file, since CSV has no binary signature); `JPEG` accepts both `jpg` and `jpeg`. `XLS` and `XLSX` retain their distinct Excel MIME types and extensions. `SupportedMediaType.spreadsheets = List(CSV, XLS, XLSX)` is used for spreadsheet uploads, while `SupportedMediaType.images` remains `List(PNG, JPEG, WEBP)`. The narrower `excel` and `csv` lists continue to drive `SpreadsheetTool.convertValidateCsv`'s branch selection.
...
`FileScanner.scan` now detects in two phases: a plain magic-only `tika.detect(tempFile)` first (unchanged, and still the only thing that ever runs for images, whose magic bytes are always unambiguous); only if that doesn't already match one of the caller's `supportedMediaTypes` does it retry, trying every accepted extension in order with a synthetic name hint (`Tika.detect(InputStream, String)`, `upload.<extension>` — Tika's own documented mechanism for exactly this "no real file name" situation), accepting only a hinted result contained in that candidate media family's MIME set.
...
- Unit: `FileScannerSpec` covers file-endpoint content sniffing without coupling to `SpreadsheetTool`: a genuine CSV file resolves via magic-only detection alone (`SupportedMediaType.CSV`, accepting Tika's `text/plain` result), and genuine `.xlsx`/`.xls` files each resolve to their real type through the hinted-retry path when no real filename is available. `SpreadsheetToolSpec` (`unit/utils/SpreadsheetToolSpec.scala`) merges both former specs' scenarios into one: first-sheet-only on a multi-sheet `.xlsx`, legacy `.xls` reads the same as `.xlsx`, well-formed CSV accepted without conversion, structurally malformed CSV rejected with `ServiceError.InternalServerError.UnexpectedError`. The dedicated schema-only spec was removed by agreement; real OpenAI schema acceptance remains the manual golden suite's responsibility.

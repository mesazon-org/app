...
4. OTP expired → delete OTP row and fail `UnauthorizedError.OtpVerificationFailedError`. Wrong OTP → `BadRequestError.OtpVerifyError`. Correct → stage set to `EmailVerified`, OTP deleted, and the `EmailVerificationVerifyOTP` wrong-attempt counter deleted.

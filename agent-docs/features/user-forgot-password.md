...
3. Expired OTP → delete + `UnauthorizedError.OtpVerificationFailedError` (HTTP: `401 UNAUTHORIZED_OTP_ERROR`). Wrong OTP → `BadRequestError.OtpVerifyError`. Correct → delete the OTP and both attempt counters.

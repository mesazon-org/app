...
- Name reusable value concepts without a feature prefix (`PriceAmount`, `PriceCurrency`, `Price`, `ImageOriginalBucketKey`, `ImageNormalizedBucketKey`, `ImageOriginalFileName`, `Image`).
- When a shared value shape has a distinct feature meaning, wrap it in an owner-specific `Pure` newtype named for the owning entity (`CatalogueItemPrice`, `CatalogueItemImage`).

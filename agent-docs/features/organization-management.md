...
1. `FileScanner.scan` spools the incoming `ZStream[Byte]` to a temp file, draining the entire input even past the byte cap (writing at most `maxFileBytes + 1` bytes, discarding the rest) so an oversized request body isn't abandoned mid-read. Detects the actual MIME type with Apache Tika (content sniffing, never the client's declared content type) and rejects anything outside `SupportedMediaTypes.images` (`PNG`, `JPEG`, `WEBP`).

...
- PostgreSQL [agnostic](standards/agnostic/postgres.md) + [project](standards/project/postgres.md) — schema/config lockstep.
- Scala [agnostic](standards/agnostic/scala.md) + [project](standards/project/scala.md) — naming for repositories, methods, params, rows, and queries.
...
- Naming and parameter conventions: [project Scala § Repository naming](standards/project/scala.md#repository-naming).
...
- Method names are **bare operation verbs** (`get`, `insert`, `update`, `delete`, `upsert`, `is...`) — no entity name (the class already carries it), plural for multi-row. Test-only helpers take a `...Testing` suffix. See [repository naming](standards/project/scala.md#repository-naming).
...
- Naming: methods **carry the entity name** (`getFooByUserID`, `insertFoo`), unlike the bare-verb query methods — [repository naming](standards/project/scala.md#repository-naming).
...
Full rule: [project Scala § Repository naming](standards/project/scala.md#repository-naming).
...
- Values holding results are named after the row: `fooBarRow`, `fooBarRowOpt`, `fooBarRows`, and a re-fetched/updated one is qualified (`fooBarRowUpdated`) — [repository naming](standards/project/scala.md#repository-naming).
...
- **Isolate**: each test samples its own data with `arbitrarySample[…]` and `.copy(…)`s only the field under test — no shared "valid X" fixtures ([Scala tests](standards/agnostic/scala.md#tests)).
...
Adding a table and adding a column both walk this same stack; see the [Postgres checklists](standards/agnostic/postgres.md#checklists).

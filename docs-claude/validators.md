...
- **Domain validators** (`validation/domain/`) for fields whose validation is effectful or non-trivial — `EmailDomainValidator` (JMail), `PhoneNumberDomainValidator` (libphonenumber). Inject these into the feature validator; wrap their output into the feature's newtype (`CustomerEmail.assume(email.value)`, `CustomerPhoneNumber(phoneNumber)`).
...
final class CustomerBookRequestValidator(
  ...
  // batch reuses the item validator + accumulates across the list
  ...
  private def validateInsertCustomerIndividuals(
    ...
    ZIO.foreach(requests)(validateInsertCustomerIndividual).map(_.sequence)
}
...
3. **Accumulate, never fail-fast.** Compose fields with `ValidatedNec` + `mapN`; compose lists with `ZIO.foreach(...).map(_.sequence)`. A caller sees *all* the bad fields in one `ValidationError`, in field order.

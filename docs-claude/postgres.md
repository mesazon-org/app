...
`jsonb` columns map to Scala `List[...]` fields via `jsonbMeta` (also in `queries.scala`): a jsoniter `JsonValueCodec` plus a `Meta` built on `PGobject`, one **explicitly named** given pair per list type (anonymous givens of the same shape collide and shadow each other). Example: `organizationEmailEntryRequestsMeta: Meta[List[OrganizationEmailEntryRequest]]`.

...
- Name such constraints `constraint uq_<table>_<distinguishing_columns> unique (...)` — e.g. `uq_customer_business_details_business_name`, `uq_customer_business_contact_email`, `uq_customer_business_contact_phone_number`. Keep the name **stable**: the repository `match`es on the literal string, so renaming the constraint without updating the repository silently drops it to the generic fallback.
...
- ❌ an anonymous `unique (organization_id, full_name)` that the repository nonetheless tries to translate by name; matching on a truncated autogenerated name like `customer_business_contact_organization_id_customer_id_phone_num`
...
- **Names are unique per tenant** — `uq_customer_business_details_business_name` / `uq_customer_individual_details_full_name` (`unique (organization_id, <name>)`) on the respective detail tables; scoped by `organization_id`, so uniqueness is per-organization, never global. A business's contacts are additionally unique on email and phone within that business — `uq_customer_business_contact_email` / `uq_customer_business_contact_phone_number` (`unique (organization_id, customer_id, email | phone_number_e164)`). All four are **named** so the repository maps their violations to a specific 409 (see [Constraint naming & conflict mapping](#constraint-naming--conflict-mapping)).

...
- **Adding a spec:** write `@DoNotDiscover class XApiSpec extends GatewayAcceptanceTest, <arbitraries>`, then add `new XApiSpec` to the `Suites(...)` list in `GatewayAcceptanceSpec`. If it needs a client/queries not already there, add the field to `GatewayItContext` and wire it in `GatewayAcceptanceSpec.buildContext`.

...
Black-box tests of the **real running gateway**: the app and all its dependencies run in docker compose (`backend/gateway/it/compose.yaml` — gateway, postgres + flyway migrations, mailhog, wiremock, s3), and tests drive it purely over HTTP. Nothing is mocked inside the application; if a test passes here, the wiring, smithy/Tapir serialization, middleware, config, and SQL all work together. Every completed feature has one spec here (linked from its doc in `docs-claude/features/`).

## Harness

The harness classes live in the **`io.mesazon.gateway.it.harness`** sub-package (`GatewayAcceptanceSpec`, `GatewayAcceptanceTest`, `GatewayItContext`, and the `appNameLive` helper); the `<Feature>ApiSpec`s stay in `io.mesazon.gateway.it`.

**One shared docker-compose stack for the whole `it` run.** A single ScalaTest aggregator, `GatewayAcceptanceSpec` (a `Suites(...)` that extends `DockerComposeBase`), is the only discoverable suite. It boots the stack **once**, nests every `<Feature>ApiSpec`, waits once (`afterContainersStart`) for gateway readiness **and** every table in `repositoryConfig.allTableNames` to exist, builds the shared `GatewayItContext`, and injects it into each nested spec via `setContext`. Booting one stack instead of one-per-spec is what keeps CI stable — the old per-spec `DockerComposeBase` mixin stood up ~8 full stacks and starved the runner (readiness timeouts, dropped keep-alive connections).

- Specs are named `<Feature>ApiSpec` (e.g. `UserSignInApiSpec`, `FileApiSpec`), are annotated **`@DoNotDiscover`**, and mix in **`GatewayAcceptanceTest`** plus their own arbitrary traits (`SmithyArbitraries`, `RepositoryArbitraries`, `IronRefinedTypeTransformer`, feature-specific ones). They do **not** extend `DockerComposeBase` and define no `Context`/`withContext`/`exposedServices`/`beforeAll`/`beforeEach` of their own — all of that is global now.
- **Running them:** because the specs are `@DoNotDiscover`, they run only through the parent — `sbt gateway-it/test` (or `testOnly *GatewayAcceptanceSpec`). `testOnly *CustomerBookApiSpec` will **not** run in isolation.
- **Sequential execution is mandatory** (`Test / parallelExecution := false` on `gateway-it` in `build.sbt`): all specs share one database, so denying sbt a `Distributor` keeps the nested suites from running concurrently against it.
- **Adding a spec:** write `@DoNotDiscover class XApiSpec extends GatewayAcceptanceTest, <arbitraries>` (importing `io.mesazon.gateway.it.harness.GatewayAcceptanceTest`), then add `new XApiSpec` to the `Suites(...)` list in `GatewayAcceptanceSpec`. If it needs a client/queries not already there, add the field to `GatewayItContext` and wire it in `GatewayItContext.build`.
...
- **Arrange and assert directly against Postgres**: `withContext { context => import context.* ... }` hands back the shared `GatewayItContext` with `PostgreSQLTestClient` plus the production `*Queries` classes — rows are seeded with `arbitrarySample[XxxRow].copy(...)` and inserted via the real insert queries; side effects are read back with the `getAll*Testing` queries.
- Emails are asserted through `MailHogClient`; S3 objects through the `s3-test` module.
- The readiness/table-existence wait lives in the parent's `afterContainersStart` (the gateway can report ready before Flyway has finished migrating; without the table wait the first `beforeEach` truncate aborts on a slow machine — `relation "local_schema.user_credentials" does not exist`). `GatewayAcceptanceTest.beforeEach` then gives **every test full isolation across the shared stack**: using the injected `context`, it truncates **all** tables (`repositoryConfig.allTableNames`) **and** resets each external service (`mailHogClient.clearInbox()`, `s3TestClient.emptyAllBuckets()`) — tests are independent and order-insensitive. (`beforeEach` is a per-suite hook, so it lives in the trait each spec mixes in — a `Suites` parent has no tests of its own, so its `beforeEach` would never fire for the nested specs.)

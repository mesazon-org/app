...
- Uniqueness via `unique (...)`: single (`unique (email)`, `unique (slug)`) or composite (`unique (user_id, action_attempt_type)`). **Name a unique constraint whenever the repository turns its violation into a distinct error** (see [Constraint naming & conflict mapping](#constraint-naming--conflict-mapping)); an anonymous `unique (...)` is fine only when a violation is a plain 500.
...
When a `unique` violation should surface to the client as a **specific 409** (not a generic 500), the repository maps it by the **violated constraint name** — so that constraint must have a **stable, explicit name**, not Postgres's autogenerated one (which also truncates at 63 chars and would silently stop matching).

- Name such constraints `constraint uq_<table>_<distinguishing_columns> unique (...)` — e.g. `uq_customer_name`, `uq_customer_business_contact_email`, `uq_customer_business_contact_phone_number`. The same applies to a **partial unique index** whose violation is client-facing (`create unique index uq_customer_name on customer (...) where status = 'Active'`) — name it explicitly. Keep the name **stable**: the repository `match`es on the literal string, so renaming the constraint without updating the repository silently drops it to the generic fallback.
- The repository reads the name off the Postgres error (`PSQLException.getServerErrorMessage.getConstraint`, SQL state `23505`) and maps it to a clear message + `ServiceError.ConflictError.UniqueConstraintViolation`. See [repository.md § Error handling](../repository.md#error-handling).
- A `unique` whose violation is *not* meant to be a distinct client error can stay anonymous (`unique (...)`) — it just becomes a `RepositoryError` (500) like any other DB failure.
...
- ❌ an anonymous `unique (organization_id, customer_id, email)` that the repository nonetheless tries to translate by name; matching on a truncated autogenerated name like `customer_business_contact_organization_id_customer_id_phone_num`
...
- **Entities that can accumulate orders (or similar historical records) are soft-deleted, never hard-deleted.** They carry a `status text not null` lifecycle column mapped to an enum (`ACTIVE` | `ARCHIVED`) — set by the repository on insert (`ACTIVE`), no DB `default`, following the enum-as-text convention. "Deleting" flips `status` to `ARCHIVED`; the row stays, so every child FK keeps resolving and the archive is reversible. Reads filter on the active status by default. Applied to the `customer` table (which holds all customer data on the one row). Pure child rows that carry no orders — e.g. `customer_business_contact` — are hard-deleted instead. (`customer.status` is stored as a real PostgreSQL `customer_status` enum type rather than a text column — the one place this schema uses a native enum instead of the text-as-enum convention below; its labels are the `CustomerStatus` case names.)
...
Orders always target a `customer_id`. A **customer** (`customer`) is the single order target and carries a `customer_type text not null` discriminator (enum-as-text). **All customer data lives on the one `customer` table** — there are no per-type detail tables; the two kinds differ only in which columns are populated:

- `INDIVIDUAL` — a standalone person; `name` is their full name, `tax_id` is null.
- `BUSINESS` — a named B2B account; `name` is the company name, `tax_id` may be set, and it may own any number of `customer_business_contact` rows.

A **business contact** is a person *inside* a company (`customer_business_contact_id`, `full_name`, `role`) — **not** a customer: it has no `customer_id` of its own, is never an order target, and carries no `status`. `customer_business_contact` is keyed on `(organization_id, customer_id, customer_business_contact_id)` and FKs `customer (organization_id, customer_id)`, so a contact can only attach to a customer in the same tenant. The FK does not distinguish type — "contacts only under a business" is a service-layer convention (contacts are only created through the business paths), not a DB rule. Within an organization, an **active** customer's `(customer_type, name)` is unique — no two active individuals (or two active businesses) in the same tenant may share a name (names may repeat across tenants, across the two types, and once archived). See [Customer Book](../features/customer-book.md) for the full lifecycle.

Consequences:

- **`customer_type` is the single source of truth for the kind** — a customer is inherently one kind (one row, one `customer_type`) and can never be "both". Typed reads/updates filter on `customer_type`, so a wrong-kind lookup returns "not found". The merged single-table model was chosen over separate `INDIVIDUAL`/`BUSINESS` detail tables (an earlier design) — it removes the cross-table join on every read and the application-level "never both kinds" invariant.
...
- **Names are unique per active tenant-kind** — `uq_customer_name` is a **partial unique index** `unique (organization_id, customer_type, name) where status = 'Active'`, scoped by `organization_id`. A business's contacts are additionally unique on email and phone within that business — `uq_customer_business_contact_email` / `uq_customer_business_contact_phone_number` (`unique (organization_id, customer_id, email | phone_number_e164)`). All are **named** so the repository maps their violations to a specific 409 (see [Constraint naming & conflict mapping](#constraint-naming--conflict-mapping)).
- ✅ `customer_type text not null` and all customer columns on `customer`; `customer_business_contact` FK `(organization_id, customer_id)` → `customer`; per-active-tenant-kind unique names via a partial index
- ❌ an `is_individual boolean` (forces a migration + backfill to add a third kind), modelling business contacts as `customer` rows, a globally-unique name with no `organization_id` in the key

## Persistence code — the three layers

> This section is the schema-sync view of the code (what keeps the SQL and the row/queries in lockstep). For the repository layer as an architecture — layer responsibilities, input models vs API requests, transactions, id/timestamp generation, error mapping, wiring, and testing — see [repository.md](../repository.md).
...
`jsonb` columns map to Scala `List[...]` fields via `jsonbMeta`: a jsoniter `JsonValueCodec` plus a `Meta` built on `PGobject`, one **explicitly named** given pair per list type (anonymous givens of the same shape collide and shadow each other). The list element is the type on the `Row` field — where the repository owns `...Input` models that is the `...Input` (e.g. `customerEmailEntryInputsMeta: Meta[List[CustomerEmailEntryInput]]` in `CustomerBookQueries`), never a smithy `...Request`; only where the repo has no input models does it stay the request type (e.g. `organizationEmailEntryRequestsMeta` in the shared `queries.scala`). Codecs over a repo `...Input` element live in the **feature's `Queries` class** next to their `Row`, which must `import <Feature>Repository.*` and `import io.github.iltotore.iron.jsoniter.given` (the latter lets the jsoniter macro resolve the Iron base codecs — omitting it fails as a macro `StackOverflowError`).

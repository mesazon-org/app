...
- **Order a `should` block success-first, failures last.** Inside the section, list the happy path, then any further **success-outcome** tests — including **silent no-ops** (an archived/absent target that returns `None`/an empty list and still succeeds with `204`) — and only **after** all of them the `fail with a …` tests. A success test must never sit below a failure test in the same block: a silent no-op asserts a *successful* outcome, so it belongs with the happy paths, not among the failures. (Acceptance specs follow the same order: the happy path and any success/no-op cases precede every `fail with a[n] {smithy error}` gate.)
  - ✅ `"archiveCustomer" should { "archive an active customer …"; "return None when no customer matches the id"; "return None … when already archived" … }` — every success before any failure.
  - ❌ a `"silently do nothing … when the business is archived"` success test placed **below** the `"fail with a UniqueConstraintViolation …"` tests in the same block.
- **Build the expected value by hand — never with the same transform the code under test uses.** When the code under test maps A→B with Chimney (`.transformInto`) or any mapping, the test's *expected* B must be constructed explicitly, field by field with names shown, **not** by calling that same `.transformInto`. A shared transform makes the test tautological: a swapped or dropped field is mirrored in both the code and the expected, so a real bug passes green. In `CustomerBookServiceSpec` the expected repository input is built by a hand-written `toInsertCustomerIndividualInput(request)` helper (independent of the service's Chimney); if the service ever maps `fullName` to the wrong field the explicit expected diverges and the test fails. (Using `.transformInto` to *arrange the input you send* is fine — it's test-owned; the rule is about the *expected output* you assert.)
  - ✅ `expects(organizationID, InsertCustomerIndividualInput(fullName = request.fullName, emails = request.emails.map(e => CustomerEmailEntryInput(email = e.email, isDefault = e.isDefault)), …))`
  - ❌ `expects(organizationID, request.transformInto[InsertCustomerIndividualInput])` — the service uses that exact transform, so a mapping bug is invisible
- **A functional (service) spec tests the service's own behaviour, not a dependency's failure taxonomy.** For failure paths, test (1) a **validation failure** — proving the service wires validation in and never calls the repository (a strict mock with no expectation enforces "never reached") — and (2) **one generic repository error** propagating unchanged (`ServiceError.InternalServerError.RepositoryError(...)` → `.zioError shouldBe repositoryError`). Do **not** enumerate the specific ways a dependency can fail (a `UniqueConstraintViolation`, a particular constraint name, etc.) — the service treats every repository error the same (it just propagates), so those belong in the repository's own integration/acceptance specs, not here. Test a dependency's specific error only when the service **handles that case differently** (e.g. the reads turn a repository `None` into an `UnexpectedError` — that branch is the service's own logic and is tested).

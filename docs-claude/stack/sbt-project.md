...
Concrete values that fill in the placeholders in [sbt-new.md](sbt-new.md). Not a standard on its own — read each fact against the rule it instantiates there.

- The build runs on **sbt 2.0.2** (a Scala 3 metabuild — everything under `project/` and `build.sbt` is Scala 3 syntax, formatted with the `scala3` scalafmt dialect). Scala version for all modules: **3.8.4**, JDK 25 (temurin, see `.tool-versions`).

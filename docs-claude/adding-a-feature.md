...
Older features left their pieces scattered (newtypes in `gateway.scala`, per-request `ServiceValidator` classes, arbitraries in the shared traits). Consolidating a feature `<Feature>` means moving every piece into the per-feature files above **without changing behavior**. Done for: `CustomerBook` (built this way), `OrganizationManagement`, `UserOnboard`, `UserSignUp`, `UserSignIn`. The steps, in order:

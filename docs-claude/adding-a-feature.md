...
| Concern | File | Notes |
|---|---|---|
| **API contract** | `backend/gateway/core/src/main/smithy/<Feature>Service.smithy` + `smithy/domain/<Feature>.smithy` | Operations + request/response shapes. See [Smithy rules](standards/agnostic/smithy.md). |
| **Domain models** | `backend/domain/src/main/scala/io/mesazon/domain/gateway/<Feature>.scala` | The entry/value case classes and the refined case classes each request validates into — **not** newtypes (all newtypes live in the single shared `Newtypes.scala`, see below) and **not** enums or other models broader than one request (each gets its own file, e.g. `OnboardStage.scala`). See [newtype rules](standards/agnostic/iron.md). |
...
| **Persistence** | `Row → Queries → Repository` (+ `RepositoryConfig`, `application.conf`) | See the [new-table checklist](standards/agnostic/postgres.md#checklists). |

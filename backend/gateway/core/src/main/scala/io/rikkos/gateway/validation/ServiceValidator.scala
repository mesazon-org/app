    ...
    case (phoneRegion, phoneNationalNumber) =>
      (for {
        _ <- if (config.supportedRegions.contains(phoneRegion.trim.toUpperCase)) ZIO.unit
          ...
          NonEmptyChain(
            (phoneRegionFieldName, s"Phone region [$phoneRegion] provided is not supported, supported regions ${config.supportedRegions.mkString("[", ",", "]")}"),
        ...
        phoneNumberRaw <- ZIO
          ...
          .flatMapError(error =>
            ZIO.fiberId.flatMap(fid =>
              ...
              NonEmptyChain(
                (phoneNationalNumberFieldName, s"Phone national number [$phoneNationalNumber] provided with region [$phoneRegion] failed to parse, supported regions ${config.supportedRegions.mkString("[", ",", "]")}"),
              )
            )
          )
        phoneNumberE164 <- if (phoneNumberUtil.isValidNumber(phoneNumberRaw)) ZIO.attempt(phoneNumberUtil.format(phoneNumberRaw, PhoneNumberFormat.E164)).orDie
          ...
          NonEmptyChain(
            (phoneNationalNumberFieldName, s"Phone national number [$phoneNationalNumber] raw [${phoneNumberRaw}] provided with region [$phoneRegion] failed to be validated, supported regions ${config.supportedRegions.mkString("[", ",", "]")}"),
          )
        )
        phoneNumber <- ZIO

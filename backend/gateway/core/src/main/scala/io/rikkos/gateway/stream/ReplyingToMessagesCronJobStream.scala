  ...
  private final class ReplyingToMessagesCronJobStreamImpl(
    ...
    private def replyToMessages(): Stream[Throwable, Unit] =
      for {
        ...
        maybeLastUserMessage = userMessages.sortBy(_.createdAt.value).lastOption
        _ <- ZStream.fromZIO(
        ...
        assistantResponse <- ZStream.fromZIO(
          openAIClient.sendMessage[AssistantResponse](
            userMessages.map(row =>
              if (row.isAssistant) Message.AssistantMessage(content = row.message)
              else Message.UserMessage(content = Content.TextContent(row.message))
            )
          )
        )
          ...
          wahaClient
            .chattingSendMessage(
              ChattingMessageInput.Text(
                sessionID = waha.SessionID.assume("testing"),
                chatID = wahaUser.wahaChatID,
                text = waha.MessageText.assume(assistantResponse.message.value),
                linkPreview = None,
        ...
        _ <- ZStream.fromZIO(
          repository.insertWahaUserMessage(
            ...
            assistantResponse.message.value,
            isAssistant = true,
          )
        )
        _ <- ZStream.fromZIO(

...
import io.rikkos.domain.waha.output.ChattingSendMessageOutput
...
package object mock extends ZIOTestOps {
  ...
  def phoneNumberValidatorMockLive(): ULayer[DomainValidator[PhoneNumberParams, PhoneNumberE164]] =
    ZLayer.succeed(
      new DomainValidator[PhoneNumberParams, PhoneNumberE164] {
        override def validate(rawData: PhoneNumberParams): UIO[ValidatedNec[InvalidFieldError, PhoneNumberE164]] =

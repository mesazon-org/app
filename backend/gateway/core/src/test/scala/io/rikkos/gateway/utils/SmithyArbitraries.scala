  ...
  given Arbitrary[smithy.WahaMessageTextRequest] = Arbitrary {
    for {
      ...
      d = smithy.WahaMessageTextRequest(payload =
        smithy.Payload(
          wahaMessage.wahaMessageID.value,
          wahaMessage.wahaUserAccountID.value,
          "Body",
          smithy.InternalData(
            smithy.InternalInfo(
              wahaMessage.wahaChatID.value,
              wahaMessage.wahaWhatsAppPhoneNumber.value,
              wahaMessage.wahaFullName.value,
            )
          ),
        )
      )

...
/** Mixed into every gateway acceptance spec. The parent [[GatewayAcceptanceSpec]] boots the one shared stack and
  * injects the context (via [[setContext]]) before the nested specs run, so a spec only calls `withContext` and gets a
  * clean DB and external state before each of its tests. Specs are `@DoNotDiscover` — they run only through the parent
  * suite.
  */

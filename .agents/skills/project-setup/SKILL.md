...
Idempotent machine bootstrap for building/running Mesazon and using the `/feature` agent pipeline. This is the "just do it" version of [`docs/repository-setup.md`](../../docs/repository-setup.md) and [`docs-Codex/agent-pipeline-setup.md`](../../docs-Codex/agent-pipeline-setup.md) — read those for the *why* behind each step; keep all three in sync if one changes.
...
9. **Codex ↔ OmniRoute wiring**: `grep -q 'ANTHROPIC_BASE_URL="http://localhost:20128"' ~/.zshrc`
10. **Interactive-tool permissions** (only matters if running an autonomous/"don't ask" permission mode): check `.Codex/settings.local.json`'s `permissions.allow` list for `AskUserQuestion`, `WebFetch`, and `WebSearch`
...
export ANTHROPIC_API_KEY="" # must be explicitly empty, or Codex may prefer it over ANTHROPIC_AUTH_TOKEN
export CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1
```
**Also required**: once OmniRoute has more than one provider serving the same bare model name (its built-in `cc` tier alongside the `Codex` connection just added), Codex's alias resolution (`opus` → `Codex-opus-4-8`) becomes ambiguous and fails with `API Error: 400 Ambiguous model... Use provider/model prefix`. Look up the exact `cc/`-prefixed IDs (`OMNIROUTE_API_KEY=<their-key> omniroute models`, they drift as new Codex models ship) and pin them:
```sh
export ANTHROPIC_DEFAULT_OPUS_MODEL="cc/Codex-opus-4-8"     # match to whatever `omniroute models` actually lists
export ANTHROPIC_DEFAULT_SONNET_MODEL="cc/Codex-sonnet-5"
export ANTHROPIC_DEFAULT_HAIKU_MODEL="cc/Codex-haiku-4-5-20251001"
```
Remind them these env vars only take effect after restarting Codex (env is read once at startup).

### 7. Interactive-tool permissions (autonomous/"don't ask" modes only)

Skip this step entirely if the engineer isn't running Codex in an autonomous permission mode — most won't need it. If `.Codex/settings.local.json` has `permissions.defaultMode` set to something autonomous (e.g. `dontAsk`) and `permissions.allow` doesn't already include `AskUserQuestion`, `WebFetch`, and `WebSearch`, add them — otherwise the Engineering Manager can't ask clarifying questions and any role that needs to look something up on the web will be silently denied (`Permission to use AskUserQuestion has been denied because Codex is running in don't ask mode`). This file is gitignored per-machine, so this has to be applied on every engineer's own setup, not just once. Note this is unrelated to the separate heuristic "auto mode classifier" safety layer, which isn't configurable this way and may still occasionally block an action.
...
Print a final table: tool → outcome (already satisfied / installed just now / needs manual follow-up, e.g. "open Docker Desktop once" or "restart Codex"). Make the remaining manual steps impossible to miss — those are the only things left for the engineer to do by hand.
...
- If you change a step here, check whether `docs/repository-setup.md` or `docs-Codex/agent-pipeline-setup.md` also needs updating — the three are meant to stay consistent.

...
10. **Interactive-tool permissions** (only matters if running an autonomous/"don't ask" permission mode): check Claude-specific `.claude/settings.local.json` for `AskUserQuestion`, `WebFetch`, and `WebSearch`
